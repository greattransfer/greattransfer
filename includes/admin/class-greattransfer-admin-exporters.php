...
class GreatTransfer_Admin_Exporters {
	...
	public function init() {
		...
		foreach ( $post_types as $post_type ) {
			...
			/* translators: Post labels singular_name */
	...
	public function admin_scripts() {
		...
		$version = defined( 'GREATTRANSFER_VERSION' ) ? GREATTRANSFER_VERSION : null;
		wp_register_script(
		...
		wp_localize_script(
			'greattransfer-post-export',
			'greattransfer_post_export_params',
			array(
				'post_type'    => $post_type,
				'export_nonce' => wp_create_nonce( 'greattransfer-' . $post_type . '-export' ),
			)
		);
	}
	...
	public function do_ajax_post_export() {
		...
		$post_type = isset( $_POST['post_type'] ) ? greattransfer_clean( $_POST['post_type'] ) : null;
		check_ajax_referer( 'greattransfer-' . $post_type . '-export', 'security' );

		if ( ! $this->export_allowed( $post_type ) ) {
			$post_type_object = get_post_type_object( $post_type );
		...
		$step     = isset( $_POST['step'] ) ? absint( greattransfer_clean( $_POST['step'] ) ) : 1;
		$exporter = new GreatTransfer_Post_CSV_Exporter( $post_type );

		$selected_post_statuses = null;
		if ( isset( $_POST['selected_post_statuses'] ) ) {
			$selected_post_statuses = wp_unslash( greattransfer_clean( $_POST['selected_post_statuses'] ) );
		}
		if ( ! empty( $selected_post_statuses ) ) {
			$exporter->set_post_statuses_to_export( $selected_post_statuses );
		}

		$columns = isset( $_POST['columns'] ) ? wp_unslash( greattransfer_clean( $_POST['columns'] ) ) : null;
		if ( ! empty( $columns ) ) {
		...
		if ( isset( $_POST['selected_columns'] ) ) {
			$selected_columns = wp_unslash( greattransfer_clean( $_POST['selected_columns'] ) );
		}
		...
		if ( isset( $_POST['export_meta'] ) ) {
			$export_meta = wp_unslash( greattransfer_clean( $_POST['export_meta'] ) );
		}
		...
		if ( isset( $_POST['export_taxonomies'] ) ) {
			$export_taxonomies = wp_unslash( greattransfer_clean( $_POST['export_taxonomies'] ) );
		}
		if ( ! empty( $export_taxonomies ) ) {
			$exporter->set_post_taxonomies_to_export( $export_taxonomies );
		}

		$filename = isset( $_POST['filename'] ) ? wp_unslash( greattransfer_clean( $_POST['filename'] ) ) : null;
		if ( ! empty( $filename ) ) {
